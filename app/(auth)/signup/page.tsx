...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nsName || !phone || !password) {
      setError('NS Name, phone, and password are required');
      return;
    }
    setLoading(true);
    setError('');
      ...
      <div className="w-full max-w-sm sm:max-w-md space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-[var(--secondary-foreground)] text-sm sm:text-base mb-2">
            Sign up to start requesting or delivering food
          </p>
        </div>
        ...
        <div className="relative py-2">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="bg-[var(--background)] px-4 text-[var(--muted)]">or sign up with password</span>
          </div>
        </div>
