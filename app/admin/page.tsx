...
export default function AdminPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-6 sm:px-8 space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Admin Dashboard</h1>
        <p className="text-[var(--secondary-foreground)] mt-2 text-base">
          {payouts.length} pending payout{payouts.length !== 1 ? 's' : ''}
        </p>
        <div className="mt-4 p-4 rounded-lg bg-[var(--card)] border border-[var(--border)] text-sm text-[var(--secondary-foreground)]">
          <p className="font-medium text-[var(--foreground)] mb-1">How payouts work</p>
      ...
      {payouts.length === 0 ? (
        <Card>
          <p className="text-center text-[var(--muted)] py-8">No pending payouts</p>
        </Card>
      ) : (
        payouts.map((payout) => (
